...
const MapBox = ({ name }) => {
  ...
  const consoleLog = (location) => {
    ...
    console.log('viewportSize: ', Object.values(viewportSizeRef.current));
    console.log('viewport width: ', viewportSizeRef.current.width);
    console.log('mapSize: ', Object.values(mapSizeRef.current));
    console.log('map width: ', mapSizeRef.current.width);
  ...
  const updateBoundaries = () => {
    boundariesRef.current = {
      minX: (viewportSizeRef.current.width - (mapSizeRef.current.width)) / 2,
      maxX: ((mapSizeRef.current.width) - viewportSizeRef.current.width) / 2,
      minY: (viewportSizeRef.current.height - (mapSizeRef.current.height)) / 2,
      maxY: ((mapSizeRef.current.height) - viewportSizeRef.current.height) / 2,
    };
  };
  ...
  useEffect(() => {
    ...
    const viewportObserver = new ResizeObserver(() => { 
        ...
        mapSizeRef.current = {
          width: (layersRef.current.offsetWidth * zoom),
          height: (layersRef.current.offsetHeight * zoom)
        };
        scaleRef.current = layersRef.current.offsetWidth / mapData.size.width;
        updateBoundaries();
